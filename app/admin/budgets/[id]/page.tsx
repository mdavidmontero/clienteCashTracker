...
export default async function BudgetDetailsPage({
  ...
  const budget = await getBudget(+params.id);
  const totalsSpent = budget.expenses.reduce(
    (total, expense) => +expense.amount + total,
    0
  );
  const totalsAvailable = +budget.amount - totalsSpent;
  const percentage = +((totalsSpent / +budget.amount) * 100).toFixed(2);
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 mt-10">
            ...
            <div className="flex flex-col justify-center items-center md:items-start gap-5">
              ...
              <Amount label="Disponible" amount={totalsAvailable} />
              <Amount label="Gastado" amount={totalsSpent} />
            </div>
          </div>
